...
def find_right_lung_lbp(image):
    right_lung = cv2.CascadeClassifier(os.path.join(os.path.split(__file__)[0], 'right_lung_lbp.xml'))
...
def find_left_lung_lbp(image):
    left_lung = cv2.CascadeClassifier(os.path.join(os.path.split(__file__)[0], 'left_lung_lbp.xml'))
